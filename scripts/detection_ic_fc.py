...
def IC_FC_detection(data, scale = 10, thres = 0.65):
    """
    This function will do two continuous wavelet tranformations and will detect IC and FC respectively by the peak times detected.
    * Data: is the data we want to do the CWT over
    * Scale: is the scale that we want to use to extract a specific frequency
    * Thres: is the threshold used for detecting peaks
    ...
    final = coefs[0]
    IC = peakutils.indexes(-final, thres, min_dist = 25)  #predefined interval from 0.25 to 2.25s
    
    #FC
    coefs_2, _ = pywt.cwt(final, scale, wavelet)
    final_2 = coefs_2[0]
    FC = peakutils.indexes(final_2, thres, min_dist = 25)  #predefined interval from 0.25 to 2.25s
...
def optimize_IC_FCs(IC,FC):
    ...
    3) It will loop through most of the ICs and FCs (there are breaks that can happen)
    4) In case there is no new FC, the loop breaks
    5) The new IC has to be bigger than last FC and should be between 0.25 and 2.25 seconds after the last IC
    6) In case there is no new IC, the loop breaks
    7) The new FC has to be bigger than last IC and should be between 0.25 and 2.25 seconds after the last FC
    8) In case an IC finished without a corresponding FC, we take away that IC
...
def identify_frequency(aV):
    """
    This function will identify the main frequency that the data contains. It is optimized to identify in walking bouts from accelerometer' measurements.
    """
    result = 0
    for cut in range(0,300,50):
        ...
        # We want to get the first peak as it should symbolize the first main frequency
